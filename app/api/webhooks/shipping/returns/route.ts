...
/**
 * Production Shiprocket Returns Webhook Route (without "shiprocket" in URL)
 * 
 * Shiprocket blocks URLs containing "shiprocket", so we expose this safe alias.
 * Public entry point for Shiprocket returns webhooks.
 * 
 * Logic:
...
export async function POST(req: NextRequest) {
  try {
    ...
    if (isTest) {
      console.log("Shiprocket returns test/verification accepted");
    ...
    if (sig) {
      ...
      return await shiprocketReturnsPostHandler(reconstructedReq);
    }
