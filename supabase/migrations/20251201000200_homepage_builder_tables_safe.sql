-- Safe Homepage Builder Tables Migration (Idempotent)
-- Can be run multiple times without errors
-- Run this INSTEAD of the previous homepage builder migrations if they failed
...
-- RLS Policies (Service Role Only) - Using DO blocks for idempotency
...
BEGIN
  IF NOT EXISTS (
    ...
    AND tablename = 'homepage_hero' 
    AND policyname = 'Service role has full access to homepage_hero'
  ) THEN
...
BEGIN
  IF NOT EXISTS (
    ...
    AND policyname = 'Service role has full access to homepage_categories'
  ) THEN
...
BEGIN
  IF NOT EXISTS (
    ...
    AND policyname = 'Service role has full access to homepage_sections'
  ) THEN
...
BEGIN
  IF NOT EXISTS (
    SELECT 1 FROM pg_policies 
    WHERE schemaname = 'public' 
    AND tablename = 'homepage_section_products' 
    AND policyname = 'Service role has full access to homepage_section_products'
  ) THEN
...
BEGIN
  IF NOT EXISTS (
    SELECT 1 FROM pg_policies 
    WHERE schemaname = 'public' 
    AND tablename = 'homepage_banners' 
    AND policyname = 'Service role has full access to homepage_banners'
  ) THEN
...
BEGIN
  IF NOT EXISTS (
    ...
    AND policyname = 'Service role has full access to homepage_settings'
  ) THEN
